...
app.get("/oauth2callback", async (req, res) => {
  ...
  try {
    ...
    res.send(
      `Token de Acesso: ${tokens.access_token} <br> Refresh Token: ${tokens.refresh_token}`,
    );
  } catch (error) {
...
async function getEmails() {
  ...
  // Busca os e-mails não lidos na caixa de entrada
  ...
  for (const message of messages) {
    ...
    const from = fromHeader ? fromHeader.value : "Remetente desconhecido";
...
bot.start((ctx) => {
  ctx.reply(
    "Olá! Eu sou seu bot para ler e-mails. Use o comando /ler para ver seus e-mails.",
  );
});
